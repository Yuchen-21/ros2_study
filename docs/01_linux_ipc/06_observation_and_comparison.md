...
> **初学者读法**：这一章是阶段总结。第一遍先读“0. 先记三件事”“4. IPC 对比”和“8. 真实机器人选型框架”。工具命令是第二遍按需练习的，不要求一次全部安装或记住。工具名和指标见[术语表](../00_glossary.md)。

## 0. 这一章先记三件事

1. 每个工具只能看到系统的一部分：抓到网络包，不代表 ROS 2 回调已经运行；看到进程活着，也不代表它没有在等待。
2. 比较通信方法不能只问“谁最快”，还要问数据大小、是否跨机器、能否丢失、接收方变慢时怎么办。
3. 先定义要观察的问题，再选择工具；不要因为熟悉某个命令就让它回答自己看不到的问题。

如果第一次学习只带走一句话，请记住：**证据也有边界。**
...
> 不存在脱离场景的“最快通信”。先弄清数据要去哪里、能不能丢、旧数据还有没有用，再选择方法和观察工具。
