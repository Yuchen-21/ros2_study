# Chapter 1 — 进程、线程、虚拟内存与上下文切换

重要度：⭐⭐⭐

> **初学者读法**：第一遍只读“0. 先记三件事”“2. 生动例子”和“6. 动手实验”。看到 MMU、page fault、futex 等词可以先跳过，所有常用词都能在[术语表](../00_glossary.md)中查询。

## 0. 这一章先记三件事

1. **同一进程的线程通常能直接看到同一个对象**，所以传数据方便，但同时修改时需要加锁等协调手段。
2. **不同进程默认看不到对方的普通变量**。即使打印出的地址文字相同，也可能对应各自不同的内存。
3. **数据已经准备好，不代表处理它的线程立刻运行**。Linux 调度器还要决定谁先获得 CPU。

本章第一次会用到：进程（process）、线程（thread）、虚拟地址（virtual address）、系统调用（system call）和上下文切换（context switch）。现在理解中文意思即可。

## 1. 为什么需要先讲这些

先不考虑 ROS 2：如果两项工作在同一进程里，它们可以通过线程共享对象；如果拆成两个进程，就需要专门的通信方法。

映射到 ROS 2 后，一个节点（node）可以单独放在一个进程，也可以通过组件组合（composition）与其他节点放进同一进程。执行器（Executor）也可能使用一个或多个线程。这些选择会影响数据是否需要跨进程搬运、回调是否可能同时运行，以及一个模块崩溃会不会带走另一个模块。
...
### 3.3 用户态（User space）与内核态（Kernel space）
...
如果能用自己的话说成“线程通常共用一间房，进程默认各有一间房；有数据等着处理，不代表人马上能进房间工作”，就已经掌握了第一遍所需内容。
