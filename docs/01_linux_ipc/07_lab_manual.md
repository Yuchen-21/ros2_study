...
> **这是一份按需查阅的操作手册，不要求一次通读 8 个实验。** 初学者先完成 [30 分钟入口](../00_beginner_path.md)中的 Lab 1～3 对应程序；学到 UDS、TCP/UDP 和共享内存章节时，再回来做相应实验。命令里的陌生词可查[术语表](../00_glossary.md)。

建议顺序：

| 学习轮次 | 做哪些实验 | 目标 |
|---|---|---|
| 第一次 | Lab 1～3 | 看懂线程共享、进程隔离和 pipe 传数据 |
| 第二次 | Lab 4～7 | 比较 UDS、TCP、UDP 和共享内存 |
| 学完整阶段后 | Lab 8、故障注入、观察工具 | 验证理解并练习排查 |

每个实验第一次只运行“运行命令”并核对“预期输出”。`strace`、`ss`、`lsof`、`/proc` 等“观察”小节都可以第二遍再做。

本章把前六章变成一组可重复实验。第二遍起不要跳过“猜想”：先写预期，再运行，最后用另一种观察方式验证。
