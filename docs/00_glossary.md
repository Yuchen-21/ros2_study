...
| 术语 | 白话解释 | 容易混淆的地方 |
|---|---|---|
| FSM（Finite State Machine，有限状态机） | 用有限状态和转移规则描述系统现在在哪一步 | 业务状态不等于 ROS 节点生命周期状态 |
| State / Event（状态 / 事件） | 状态是“正在配送”，事件是“刚收到取消请求” | 同一事件在不同状态可能有不同处理 |
| Guard（守卫条件） | 允许转移前必须满足的条件，例如电量足够 | 条件值变化不一定自动产生事件 |
| Transition（转移） | 从一个状态按规则走到另一个状态 | 改本地状态不代表外部设备动作已经完成 |
| HSM（Hierarchical State Machine，分层状态机） | 用父子状态表达共同规则与内部模式 | 需要明确父子层处理事件的优先级 |
| Lifecycle（节点生命周期） | 统一管理节点的配置、激活、停用和清理 | 不管理每张订单的业务进度 |
| LifecycleNode（生命周期节点） | 提供标准状态转换与管理接口的 ROS 2 节点 | inactive 不自动冻结全部普通回调 |
| LifecyclePublisher（生命周期发布者） | 未激活时阻止发布的受管理发布者 | 不会自动停止所有 timer 或底层硬件 |
| BT（Behavior Tree，行为树） | 用树形结构组合条件、任务和候选策略 | 一个 BT 节点不等于一个 ROS node |
| Tick（推进一轮） | 检查或推进树的执行一次 | 不是一个 OS 线程，也不保证固定时间周期 |
| SUCCESS / FAILURE / RUNNING | 树节点成功、失败或尚未结束 | ROS action 接受目标不等于树动作成功 |
| Sequence / Fallback | 按顺序完成任务 / 依次试候选策略 | RUNNING 时如何续跑取决于具体控制节点 |
| ReactiveSequence（响应式顺序节点） | 每轮重查前面的条件，必要时中断后面的任务 | 普通 Sequence 不必具有相同重查语义 |
| Halt（中断） | 树通知运行分支停止并处理自身状态 | ROS 远端取消仍需请求和确认 |
| Blackboard / Ports（黑板 / 端口） | 树节点共享上下文 / 声明输入输出 | 不自动保证数据新鲜或线程安全 |
| Decorator（装饰节点） | 给一个子节点附加超时、重试等执行规则 | 无限重试可能掩盖故障，超时不等于远端已停 |

配套教程：[状态机](16_state_machine/README.md)、[Lifecycle](17_lifecycle/README.md)、[行为树](18_behavior_tree/README.md)。
