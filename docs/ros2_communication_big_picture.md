...
> **初学者提示**：这是一张“学完后逐渐补全”的地图，不是进入仓库后必须先看懂的架构图。第一次只看下面的简化版；完整图中碰到的词可在[术语表](00_glossary.md)查询，也可以等学到对应阶段再回来。
...
接收者得知“有数据可处理”
      ↓
Executor 安排某个线程运行 callback
```

这里最容易忽略的是最后两步：**数据到达**和**处理数据的函数开始运行**不是同一件事。其余各层都是在逐步解释这四步由谁完成、数据在哪里等待，以及问题可能发生在哪里。

这张图不是“组件清单”，而是一张责任、线程、队列和延迟地图。读图时始终问：

1. 当前数据由谁负责，什么时候可以修改或释放？
2. 当前代码运行在哪个线程？
3. 数据跨过进程或机器边界了吗？
4. 数据可能暂存在什么缓冲区或队列里？
5. 谁通知“可以处理了”，又是谁真正选择并运行回调函数？
