...
本页现在是通信工程 **V2**：先完成[配送机器人 V1](../task_orchestration_capstone.md)的任务闭环，再增加下面的大消息、监控和网络实验。V1 与 V2 均尚未实现；目录 14 不是学习状态机、Lifecycle、行为树的先修门槛。顺序见[统一路线](../00_course_map.md)。

重要度：⭐⭐⭐　预计：30～50 小时　依赖：Stage 01～12

> **白话预告**：这是最后的综合练习，把传感器、定位、规划和控制连起来，再主动制造故障并用证据修好。它不是起步项目；等前面的阶段逐步完成后再进入。术语见[术语表](../00_glossary.md)。
